...
/*!
   \file constants.h
   \brief This is a collection of constants that are used throughout the
   project.
...
// _________________
// CONFIG FLAGS
/// A flag for the mode (true = manual; false = dynamic). Manual for static map
/// or dynamic for dynamic map.
...
/*!
   ...
   \brief A flag to toggle the Dubin's heuristic via lookup, potentially
   speeding up the search by a lot
...
// 2021.3 xuy, truck params constant, bbox outline
/// [m] --- The width of the vehicle
static const double width = 3.1 + 2 * bloating;
/// [m] --- The length of the vehicle
static const double length = 7.5 + 2 * bloating;
/// [m] --- wheel base
static const double wheel_base = 3.7;
/// [m] --- front axle to front
static const double front_axle_to_front = 3;
/// [m] --- back axle to back
static const double back_axle_to_back =
    length - front_axle_to_front - wheel_base;
...
/// [m] -- The bounding box size length and width to precompute all possible
/// headings
static const int bbSize =
    std::ceil(sqrt(width * width + length * length) * 2 / cellSize);
/// [#] --- The sqrt of the number of discrete positions per cell
// 2021.3 related to cellSize
static const int positionResolution = 5;
/// [#] --- The number of discrete positions per cell
static const int positions = positionResolution * positionResolution;
/// A structure describing the relative position of the occupied cell based on
...
/// [m] --- The width of the dubinsArea / 2 for the analytical solution (Dubin's
/// shot)
...
/// A definition for a color used for visualization
static constexpr color green = {16.f / 255.f, 226.f / 255.f, 46.f / 255.f};
/// A definition for a color used for visualization
...
/// A definition for a color used for visualization
static constexpr color red = {224.f / 255.f, 12.f / 255.f, 25.f / 255.f};
} // namespace Constants
} // namespace HybridAStar
