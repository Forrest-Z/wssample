...
void Smoother::smoothPath(DynamicVoronoi &voronoi) {
    ...
    // descent along the gradient untill the maximum number of iterations has
    // been reached
    ...
    while (iterations < maxIterations) {
        ...
        for (int i = 2; i < pathLength - 2; ++i) {
            ...
            // keep these points fixed if they are a cusp point or adjacent to
            // one
            if (isCusp(newPath, i)) {
                continue;
            }
            ...
            correction =
                correction - smoothnessTerm(xim2, xim1, xi, xip1, xip2);
            if (!isOnGrid(xi + correction)) {
                continue;
            }
...
Vector2D Smoother::curvatureTerm(Vector2D xim1, Vector2D xi, Vector2D xip1) {
    ...
    if (absDxi > 0 && absDxip1 > 0) {
        ...
        float Dphi = std::acos(
            Helper::clamp(Dxi.dot(Dxip1) / (absDxi * absDxip1), -1, 1));
        ...
        if (kappa <= kappaMax) {
            ...
            float PDphi_PcosDphi =
                -1 / std::sqrt(1 - std::pow(std::cos(Dphi), 2));
