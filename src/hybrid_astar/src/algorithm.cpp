...
Node3D *Algorithm::hybridAStar(Node3D &start, const Node3D &goal,
    ...
    int iPred, iSucc;
    float newG;
    // Number of possible directions, 3 for forward driving and an additional 3
    // for reversing
    int dir = Constants::reverse ? 6 : 3;
    // Number of iterations the algorithm has run for stopping based on
            ...
            // CONTINUE WITH SEARCH
                ...
                for (int i = 0; i < dir; i++) {
                    ...
                    if (nSucc->isOnGrid(width, height) &&
                        ...
                        // ensure successor is not on closed list or it has the
                        // same index as the predecessor
                        if (!nodes3D[iSucc].isClosed() || iPred == iSucc) {
                            ...
                            // if successor not on open list or found a shorter
                            // way to the cell
                            if (!nodes3D[iSucc].isOpen() ||
                                ...
                                // calculate H value
                                updateH(*nSucc, goal, nodes2D, dubinsLookup,
                                        width, height, configurationSpace,
                                        visualization);

                                // if the successor is in the same cell but the
                                // C value is larger
                                if (iPred == iSucc &&
                                    nSucc->getC() >
                                        nPred->getC() + Constants::tieBreaker) {
                                    delete nSucc;
                                    continue;
                                }
                                // if successor is in the same cell and the C
                                // value is lower, set predecessor to
                                // predecessor of predecessor
                                else if (iPred == iSucc &&
                                         nSucc->getC() <=
                                             nPred->getC() +
                                                 Constants::tieBreaker) {
            ...
            // CONTINUE WITH SEARCH
                ...
                for (int i = 0; i < Node2D::dir; i++) {
                    ...
                    if (nSucc->isOnGrid(width, height) &&
                        ...
                        // if successor not on open list or g value lower than
                        // before put it on open list
...
void updateH(Node3D &start, const Node3D &goal, Node2D *nodes2D,
    ...
    if (Constants::twoD &&
        !nodes2D[(int)start.getY() * width + (int)start.getX()]
             .isDiscovered()) {
        //    ros::Time t0 = ros::Time::now();
    ...
    if (Constants::twoD) {
        ...
        twoDoffset = sqrt(((start.getX() - (long)start.getX()) -
                           (goal.getX() - (long)goal.getX())) *
                              ((start.getX() - (long)start.getX()) -
                               (goal.getX() - (long)goal.getX())) +
                          ((start.getY() - (long)start.getY()) -
                           (goal.getY() - (long)goal.getY())) *
                              ((start.getY() - (long)start.getY()) -
                               (goal.getY() - (long)goal.getY())));
        twoDCost =
            nodes2D[(int)start.getY() * width + (int)start.getX()].getG() -
            twoDoffset;
    }
