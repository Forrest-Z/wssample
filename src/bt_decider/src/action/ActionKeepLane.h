...
class ActionKeepLane : public AsyncActionNode {
  public:
    ActionKeepLane(const string &name, const NodeConfiguration &config)
    ...
    virtual NodeStatus tick() override {
        ...
        strategy.lat_strategy = "keep_in_lane";

        pub_lat_strategy_.publish(strategy);
        ROS_INFO_STREAM("[decider] keep in this lane !");
