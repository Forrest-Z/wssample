...
class CheckLaneFrontClear : public SyncActionNode {
  public:
    CheckLaneFrontClear(const string &name, const NodeConfiguration &config)
        ...
        sub_lanes_ = node_.subscribe(lane_topic_, 100,
                                     &CheckLaneFrontClear::ProcWayPoints, this);
        sub_objs_ = node_.subscribe(objs_topic_, 100,
                                    &CheckLaneFrontClear::ProcObjs, this);
    }
