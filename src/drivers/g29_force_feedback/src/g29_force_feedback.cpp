...
void G29ForceFeedback::updateFfDevice() {
    ...
    while (read(m_device_handle, &event, sizeof(event)) == sizeof(event)) {
        if (event.type == EV_ABS && event.code == m_axis_code) {
            m_current_angle = (event.value - (m_axis_max + m_axis_min) * 0.5) *
                              2 / (m_axis_max - m_axis_min);
        }
    }
}
...
void G29ForceFeedback::targetCallback(
    const g29_force_feedback::ForceFeedback &in_target) {
...
void G29ForceFeedback::initFfDevice() {
    ...
    if (ioctl(m_device_handle, EVIOCGBIT(EV_FF, sizeof(ff_bits)), ff_bits) <
        0) {
