  ...
  while (true)
  {
    ...
    if (can_reader.isOpen())
    {
      ...
      if (ret  == ReturnStatuses::OK)
      {
        ...
        {
          // dataspeed_can_msgs::CanMessageStamped can_pub_msg;
          // can_pub_msg.header.frame_id = "0";
          // can_pub_msg.msg.id = msg.id;
          // can_pub_msg.msg.dlc = msg.data.size();
          // can_pub_msg.msg.extended = msg.flags.ext_id;
          // // can_pub_msg.msg.is_error = msg.flags.error_frame;
          // // can_pub_msg.is_rtr = msg.flags.rtr;
          // std::copy(msg.data.begin(), msg.data.end(), can_pub_msg.msg.data.begin());
...
void can_tx_callback(const can_msgs::Frame::ConstPtr& in_msg)
{
  ReturnStatuses ret;
  ...
  if (can_writer.isOpen())
  {
    ...
    msg.id = in_msg->id;
    msg.dlc = in_msg->dlc;
    msg.flags.ext_id = in_msg->is_extended;
    // msg.flags.rtr = in_msg->is_rtr;

    for (size_t i = 0; i < in_msg->data.size(); ++i)
    {
      msg.data.push_back(in_msg->data[i]);
    }
...
int main(int argc, char** argv)
{
  ...
  std::string rx_topic, tx_topic;
  priv.param<std::string>("rx_topic", rx_topic, "");
  priv.param<std::string>("tx_topic", tx_topic, "");

  can_rx_pub = n.advertise<can_msgs::Frame>(rx_topic, 500);
  ros::Subscriber can_tx_sub = n.subscribe(tx_topic, 500, can_tx_callback);
